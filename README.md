...
**Verified on this machine (polish sweep, steps 1–4, 19 Sep):** 36 tests — new: tap vs joystick vs pinch on synthetic pointer events; the toast queue; other players gliding at constant speed between updates, no jump when an update lands, lift rides placed not dragged; stands grouped from neighbouring cells; booth picking (every booth from straight above, 500+ booths from the game camera angle at four headings, never through the booth in front, open floor picks nothing). Browser run of the static build: Esc and Back close a sheet, Back on the bare game warns before leaving, the express tray closes on an outside press; hovering a booth shows the frame, the tag and the pointing hand, pressing it walks there and the Stamp button (with its E key cap) is for that booth; dock tooltips and tray key caps; at 1920 × 1000 the interface scale is 1.15, the interface still fills the screen exactly and a sheet fits inside it. **Not verified:** the 1.3 scale on a real 1440p / 4K monitor, Firefox and Safari with a mouse, any of this on a real phone.
