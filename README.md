...
**Verified on this machine (polish sweep, steps 1–7, 19 Sep):** 44 tests — new: tap vs joystick vs pinch on synthetic pointer events; the toast queue; other players gliding at constant speed between updates, no jump when an update lands, lift rides placed not dragged; stands grouped from neighbouring cells; booth picking (every booth from straight above, 500+ booths from the game camera angle at four headings, never through the booth in front, open floor picks nothing). Browser run of the static build: Esc and Back close a sheet, Back on the bare game warns before leaving, the express tray closes on an outside press; hovering a booth shows the frame, the tag and the pointing hand, pressing it walks there and the Stamp button (with its E key cap) is for that booth; dock tooltips and tray key caps; at 1920 × 1000 the interface scale is 1.15, the interface still fills the screen exactly and a sheet fits inside it. First ten seconds (step 7): the live count on the first screen ("37 in the expo now" in the demo), the landing camera move settling behind the player, the move hint appearing once and leaving after a few steps (remembered), favicon / manifest / share-card files present in the build; the card preview filling in as a name and company are typed (not submitted). Movement was moved into `src/game/mover.ts` and is now tested on the real floor plan: "take me there" from both entrances at 60, 30 and 20 frames a second, a press on 146 booths across the three levels, and 400 seeded random walks with irregular frame times — all arrive, none needs a re-plan; the browser walk to the X arrives at full speed and at an emulated 2 frames a second. **One open observation:** once, before this change, under the test browser's 1–2 fps throttling, "take me there" stopped 18 m short and a second press finished it; it has not been reproduced since (546 simulated walks, two browser runs). A route that makes no progress for half a second now plans again from where it stands, and `?perf` exposes the mover's state (`__mx.mover()`) to catch it on a phone if it is real. Performance (step 6): same scene, 20 other players: 238 → 60–83 draw calls, 203k → 72–86k triangles (read from the renderer with `?perf`); characters, crew and crowd look the same in the browser. Sound (step 5): 40 tests — silent before the first touch, one sound per moment, the menu switch silences everything and is remembered; in the browser no oscillator exists before the first click, the door click starts the audio context, a floor press plays, and switching sound off in the menu plays nothing afterwards. **Not verified:** the share card inside WhatsApp / LinkedIn (needs the deployed address), the home-screen icon on a real phone, the landing move judged by eye at full frame rate; frames per second on any phone — the numbers above are what the GPU is asked to draw, not how fast a phone draws it (open the game with `?perf` on the phone to read it); how the sounds actually sound — they were scheduled and counted, not listened to: loudness and tone need a human ear and a phone speaker; vibration on a real Android; the 1.3 scale on a real 1440p / 4K monitor, Firefox and Safari with a mouse, any of this on a real phone.
